...
from utils.icons import GIT_ICON_SET, CHECKED, UNCHECKED
...
class GitTree(GenericTree):
    ...
    if ("cat", "changes") in self._expanded:
      if not status_list:
        ...
        for i, s in enumerate(status_list):
          ...
          result.append(TreeEntry(
            node_id={"type": "change", "path": s["path"], "staged": s["staged"]},
            indent=self.VERTICAL + branch,
            is_expandable=False,
            is_expanded=False,
            display_name=label,
  ...
  def get_node_buttons(self, node_id, is_expandable) -> list:
    if isinstance(node_id, dict) and node_id.get("type") == "change":
      ...
      btn = self._make_btn(label, "Toggle for commit/stage/unstage", "toggle_select")
      return [btn]
    return []
